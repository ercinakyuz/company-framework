...
{
    public class KafkaRetrialContext : IKafkaRetrialContext
    {
        ...
        public KafkaRetrialContext(IKafkaProducer producer, ConsumerRetriability retriability, KafkaRetrySettings settings, ILogger<KafkaRetrialContext> logger)
        {
            _producer = producer;
        ...
        public async Task RetryAsync(KafkaRetryArgs args, CancellationToken cancellationToken)
        {
            if (_retriability.IsRetriableException(args.ExceptionType))
            {
                ...
                if (_settings.Count > retryAttempts)
                {
                    ...
                    await _producer.ProduceAsync(new KafkaProduceArgs(_settings.Topic, args.Message, nextHeaders), cancellationToken).ConfigureAwait(false);
